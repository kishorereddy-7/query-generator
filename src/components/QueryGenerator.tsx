...
import { ConditionTypeEnum, FieldEnum, OperatorEnum } from "../constants/query.constant";
    ...
    queryList.forEach((query, index) => {
        if (query.type === ConditionTypeEnum.GROUP) {
            query.conditions?.forEach((gQuery, gIndex) => {
                string += (gIndex === 0 ? " (" : "") + gQuery.queryString + (gIndex === ((query.conditions?.length!) - 1) ? ")" : (" " + query.logicalOperator))
            })
        } else {
            string += query.queryString + (index === (queryList.length - 1) ? "" : (" " + globalLogicalOperator))
        }
    })
      ...
      <div className="bg-black border-t border-gray-800 rounded-2xl my-4">
        <div className="container mx-auto px-4 py-3">
          <span className="text-green-400 font-mono text-sm">
            Query: <code className="ml-2">WHERE {queryString}</code>
          </span>
        </div>
      </div>
    </div>
